...
for i in range(0, t.GetEntries()):
    ...
    if len(idll) == 0:
        continue
    l1, l2, fsrph = ROOT.TLorentzVector(), ROOT.TLorentzVector(), ROOT.TLorentzVector()
    l1.SetPtEtaPhiM(t.mu_pt[x], t.mu_eta[x], t.mu_phi[x], 0.105)
    l2.SetPtEtaPhiM(t.mu_pt[y], t.mu_eta[y], t.mu_phi[y], 0.105)
    
    fsrph.SetPtEtaPhiM(t.fsrphoton_pt[0], t.fsrphoton_eta[0], t.fsrphoton_phi[0], 0)

    if t.fsrphoton_muonidx[0]==x and l1.DeltaR(fsrph) < 0.5 and fsrph.Pt()/l1.Pt() < 0.4:
        l1 = l1 + fsrph
    elif t.fsrphoton_muonidx[0]==y and l2.DeltaR(fsrph) < 0.5 and fsrph.Pt()/l2.Pt() < 0.4:
        l2 = l2 + fsrph
