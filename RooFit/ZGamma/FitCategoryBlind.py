...
def FitCategory(category = "untag4", fitType = "ModGaus", nbin = 80, xlow = 100, xhigh = 180, useSumW2 = True, defaults = []):
    ...
    data = ROOT.RooDataHist(category, category, ROOT.RooArgList(m), ROOT.RooFit.Import(hmc))
    ...
    fitResult = background.fitTo(blindedData, ROOT.RooFit.Minimizer("Minuit2", "Migrad"), ROOT.RooFit.Save(1), ROOT.RooFit.Range(xlow, xhigh), ROOT.RooFit.SumW2Error(ROOT.kTRUE))

    PlotFit(fitResult, blindedData, background, m, nbin, xlow, xhigh, outpath, True)

    PlotCovariances(fitResult, outpath)
    
    PlotLikelihood(fitResult, background, blindedData, outpath)
